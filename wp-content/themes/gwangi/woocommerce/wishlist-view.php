...
<div class="wc-up-nav bg-black-faded rounded p-3 text-center">
	<a href="<?php echo esc_url( wc_get_cart_url() ); ?>">
		<i class="gwangi-icon gwangi-cart fa-lg"></i> <?php esc_html_e('View cart', 'gwangi'); ?>
	</a>
</div>
...
<form id="yith-wcwl-form" action="<?php echo esc_url( $form_action ); ?>" method="post" class="woocommerce">
	...
	if ( ! empty( $page_title ) ) : ?>
		<?php $wishlist_class = ( $is_custom_list ) ? 'wishlist-title-with-form' : ''; ?>

		<div class="wishlist-title <?php echo esc_attr( $wishlist_class ); ?>">
	...
	<div class="card card-static p-3">
		...
		<table class="shop_table cart wishlist_table mb-0" data-pagination="<?php echo esc_attr( $pagination ); ?>" data-per-page="<?php echo esc_attr( $per_page ); ?>" data-page="<?php echo esc_attr( $current_page ); ?>" data-id="<?php echo esc_attr( $wishlist_id ); ?>" data-token="<?php echo esc_attr( $wishlist_token ); ?>">
			...
			<tr>
				...
				<th class="product-name"><span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'yith-woocommerce-wishlist' ) ); ?></span></th>

				<?php if ( $show_price ) : ?>

					<th class="product-price"><span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'yith-woocommerce-wishlist' ) ); ?></span></th>
				...
				<?php if ( $show_stock_status ) : ?>
					...
					<th class="product-stock-status"><span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'yith-woocommerce-wishlist' ) ); ?></span></th>
			...
			if ( count( $wishlist_items ) > 0 ) :
				...
				foreach ( $wishlist_items as $item ) :
					...
					if ( in_array( $item['prod_id'], $added_items, true ) ) {
						continue;
					}
						...
						<tr id="yith-wcwl-row-<?php echo esc_attr( $item['prod_id'] ); ?>" data-row-id="<?php echo esc_attr( $item['prod_id'] ); ?>">
							...
							<?php if ( $show_price ) : ?>
								<td class="product-price">
									<?php
									$price_html = $product->get_price() ? $product->get_price_html() : apply_filters( 'yith_free_text', esc_html__( 'Free!', 'yith-woocommerce-wishlist' ) );
							...
							<?php if ( $show_last_column ) : ?>
								<td class="product-add-to-cart">
									...
									if ( $show_dateadded && isset( $item['dateadded'] ) ) : ?>
										<span class="dateadded"><?php
											/* translators: %s: Date added */
											printf( esc_html__( 'Added on : %s', 'yith-woocommerce-wishlist' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ); ?></span>
										<?php
									endif; ?>
				...
				<tr>
					<td colspan="<?php echo esc_attr( $column_count ); ?>" class="wishlist-empty"><div class="text-center p-4"><i class="fa fa-frown-o fa-3x"></i><h4><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products were added to the wishlist', 'yith-woocommerce-wishlist' ) ); ?></h4></div></td>
				</tr>
				<?php
			endif;
				...
				<tr>
					<td colspan="<?php echo esc_attr( $column_count ); ?>">
						...
						<?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ) : ?>
							<?php $ask_estimate_button_url = ( $additional_info || ! is_user_logged_in() ) ? '#ask_an_estimate_popup' : $ask_estimate_url; ?>
							<?php $additional_info_attribute = ( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : ''; ?>
							<div class="ask-an-estimate-button-container">
								<a href="<?php echo esc_url( $ask_estimate_button_url ); ?>" class="btn button ask-an-estimate-button" <?php echo esc_html( $additional_info_attribute ); ?>>
